...
def test_run():
    # Define a date range
    dates = pd.date_range('2016-01-01', '2016-10-31')
    ...
    sma5_eur = math_util.get_rolling_mean(df['EUR'], window=5)
    sma20_eur = math_util.get_rolling_mean(df['EUR'], window=20)
    rstd_eur = math_util.get_rolling_std(df['EUR'], window=20)
    upper_band_eur, lower_band_eur = math_util.get_bollinger_bands(sma20_eur, rstd_eur)
    ...
    sma5_usd = math_util.get_rolling_mean(df['USD'], window=5)
    sma20_usd = math_util.get_rolling_mean(df['USD'], window=20)
    rstd_usd = math_util.get_rolling_std(df['USD'], window=20)
    upper_band_usd, lower_band_usd = math_util.get_bollinger_bands(sma20_usd, rstd_usd)
    ...
    sma5_gbp = math_util.get_rolling_mean(df['GBP'], window=5)
    sma20_gbp = math_util.get_rolling_mean(df['GBP'], window=20)
    rstd_gbp = math_util.get_rolling_std(df['GBP'], window=20)
    upper_band_gbp, lower_band_gbp = math_util.get_bollinger_bands(sma20_gbp, rstd_gbp)
